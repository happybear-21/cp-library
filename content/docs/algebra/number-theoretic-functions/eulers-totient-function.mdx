...
The multiplicative order of an element  $a$  mod  $n$ , denoted  $\operatorname{ord}_n(a)$ , is the smallest  $k>0$  such that  $a^k \equiv 1 \pmod m$ .  $\operatorname{ord}_n(a)$  is the size of the subgroup generated by  $a$ , so by Lagrange's Theorem, the multiplicative order of any  $a$  must divide  $\phi(n)$ . If the multiplicative order of  $a$  is  $\phi(n)$ , the largest possible, then  $a$  is a primitive root and the group is cyclic by definition.
