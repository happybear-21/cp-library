...
$\gcd$ . Which gives us a simple rule: if one of the numbers is zero, the greatest common divisor is the other number.
...
Note that  $a$  remains the larger number until  $b$  is subtracted from it at least  $\left\lfloor\frac{a}{b}\right\rfloor$  times. Therefore, to speed things up,  
$a-b$  is substituted with  $a-\left\lfloor\frac{a}{b}\right\rfloor b = a \bmod b$ . Then the algorithm is formulated in an extremely simple way:
