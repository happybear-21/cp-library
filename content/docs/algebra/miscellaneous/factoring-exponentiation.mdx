...
In this article, we will cover the implementation for  $32$ -bit integers. Let mbin_log_32(r, x) be a function that computes  $r+4L(x) \pmod{2^d}$ ;```mbin_exp_32(r, x)``` be a function that computes  $r b^{\frac{x}{4}} \pmod{2^d}$ ;```mbin_power_odd_32(a, x, y)``` be a function that computes  $ax^y \pmod{2^d}$ .
